...
const MARPatientStudent = mongoose.model('mar-patient-student');
	...
	.sort({'datetime': 1}).then(newFluid => {
		...
		.sort({'date': -1, 'time': 1}).then(newOtherScheduleFluid => {
			...
			res.render('charts/master/charts-fluid-regime', {
				recordID: req.params.recordID,
				newOtherScheduleFluid: newOtherScheduleFluid,
				userType: userType,
				currentUserType: req.user.userType,
				studentFirstName: req.session.firstNameAndEmail,
				newFluid: newFluid,
...
router.get('/list-mar-patients/:ward', ensureAuthenticated,(req,res) =>{
	PatientStudentModel.find({ward: req.params.ward, user: req.user._id}).then(sPatients =>{
		PatientMasterModel.find({ward: req.params.ward}).then(mPatients=>{
			// console.log("mpatients " + mPatients);
			MARPatientMaster.find({ward: req.params.ward}).then(marPatientMaster=>{
				MARPatientStudent.find({ward: req.params.ward}).then(marPatientStudent=>{
					MARCase.find({patientID: MARPatientMaster.patientID}).then(marcase =>{
						// console.log('MARCASE ', marcase);
						res.render('mar/list-patients/student-list-patients',{
							sPatients: sPatients,
							mPatients: mPatients,
							MARPatientMaster: marPatientMaster,
							MARPatientStudent: marPatientStudent,
							marcase: marcase,
							toaster: req.session.toaster
						});
						req.session.toaster = null; // reset toaster
						// console.log("spatient ward: ", sPatients.ward);
...
// Student copy of master patient
router.get('/copy-mar-patient/:patientId', ensureAuthenticated,(req,res) =>{
	MARPatientMaster.findOne({_id: req.params.patientId}).then(patient=>{
		res.render('mar/mar-patient/student-copy-patient',{
			patient: patient
		});
	})
})
...
// Student copy of master patient
router.post('/copy-mar-patient/:patientId', ensureAuthenticated,(req,res) =>{
	let {
	...
	MARCase.findOne({patientId: req.params.patientId}).then(marCase =>{
		MARPatientMaster.findOne({_id: req.params.patientId}).then(patient=>{
			...
			new MARPatientStudent({
				...
				creator: req.user.firstName,
				creatorEmail: req.user.email,
				masterPatient: patient._id
				// caseId: MARCase.marCase._id
			}).save()
			.then(newPatient=>{
				toaster.setSuccessMessage(' ', 'MAR Record Copied');
				req.session.toaster = toaster;
				res.redirect(`/student/list-mar-patients/${ward}`);
			});
		})
	})
	...
	// MARPatientMaster.findOne({patientId: req.params.patientId}).then(marpatient => {
	// 	MARCase
	// })
})

// Student edit of student patient
router.get('/edit-mar-patient/:patientId', ensureAuthenticated,(req,res) =>{
	MARPatientStudent.findOne({_id: req.params.patientId}).then(patient=>{
		res.render('mar/mar-patient/student-edit-patient',{
			patient: patient
		});
	})
})
...
// Student edit of student patient
router.post('/edit-mar-patient/:patientId', ensureAuthenticated,(req,res) =>{
	let {
		nric, familyName, givenName, gender, ethinicity, dob, weight, height, allergy,
		ward, bed, consultant, diagnosis, history
	} = req.body;
...
// Delete MAR patient (Student copy)
router.get('/delete-mar-patient/:patientId', ensureAuthenticated,(req,res) =>{
	MARPatientStudent.findOneAndDelete({_id: req.params.patientId}).then(patient=>{
		toaster.setSuccessMessage(' ', 'MAR Record Deleted');
		console.log("Patient deleted");
		req.session.toaster = toaster;
		res.redirect(`/student/list-mar-patients/${patient.ward}`)
		
	})
})

// Route for students to view sliding scale
router.get('/slidingScale/:scaleId', ensureAuthenticated,(req,res) =>{
	MARSlidingScale.find({scaleId: req.params.scaleId}).then(scale=>{
		res.render("mar/slidingScale/slidingScaleStudent",{
...
router.get('/mar-management/:patientId', ensureAuthenticated, (req,res) =>{

	PatientStudentModel.findOne({patientID: req.params.patientId}).then(mPatient=>{
			...
			MARMedication.find({caseId: marCase._id}).then(marMedication=>{
				...
				MedAdministration.find({medicationId: marMedication._id}).then(medAdmin=>{
					// If patient is created from MAR
						if(mPatient == null){
							MARPatientStudent.findOne({patientId: req.params.patientId}).then(mPatient=>{
								res.render('mar/student/mar-management',{
									medAdmin: medAdmin,
...
router.get('/serve-medication/:caseId',  ensureAuthenticated, (req, res) =>{
	DrugList.find().then(druglist =>{
		// console.log(druglist.find);
		MARCase.findOne({_id: req.params.caseId}).then(marCase =>{
			PatientStudentModel.findOne({patientID: marCase.patientId}).then(retrieveSPatient=>{
				MARMedication.find({caseId: marCase._id}).then(marMedication=>{
					console.log(marMedication);
					StudMedAdministration.find({caseId: marCase._id}).then(medAdmin=>{	
						// Retrieving records for MAR patient
						if(retrieveSPatient === null){
							MARPatientStudent.findOne({patientId: marCase.patientId}).then(retrieveSPatient =>{
								res.render('mar/student/mar-administration',{
									medAdmin: medAdmin,
									marMedication: marMedication,
									marCase: marCase,
									druglist: druglist,
									marpatient: retrieveSPatient,
									retrieveSPatient: retrieveSPatient,
						...
						//Retrieving records for EMR patient
						else{
							res.render('mar/student/mar-administration',{
								medAdmin: medAdmin,
								marMedication: marMedication,
								marCase: marCase,
								druglist: druglist,
								patient: retrieveSPatient,
								retrieveSPatient: retrieveSPatient,
								serveMedication: true,
								toaster: req.session.toaster,
								recordID: retrieveSPatient.recordID
							})
							req.session.toaster = null; // reset toaster
						}
					})
...
router.post('/add-med-adm/:medicationId', ensureAuthenticated, (req, res) =>{
	...
	MARMedication.findOne({_id: req.params.medicationId}).then(marMedication =>{
		// console.log("MAR medication id ", marMedication._id);
		MARCase.findOne({_id: marMedication.caseId}).then(marCase =>{
			// console.log("MAR Case ", marCase._id);
			StudMedAdministration.find({medicationId: marMedication._id}).then(medAdmin=>{
				if (admType == 'A'){
					new StudMedAdministration({
						...
						res.redirect(`/student/serve-medication/${marMedication.caseId}`)
					});
				} else if(admType == 'Due'){
					new StudMedAdministration({
						medicationId: admmedID,
...
// Update Student Medication Administration Case
router.post('/edit-med-adm/:admId', ensureAuthenticated, (req,res)=>{
	...
	StudMedAdministration.findOne({_id: req.params.admId}).then(editadm =>{
		// console.log("Student Administration ID: ", editadm._id);
		if(admType == 'A'){
		...
		editadm.save().then(savedadm =>{
			res.redirect(`/student/serve-medication/${savedadm.caseId}`)
		})
	})
})

//Delete Student Medication Administration Case
router.get('/delete-med-adm/:admId', ensureAuthenticated, (req, res)=>{
	StudMedAdministration.findOneAndDelete({_id: req.params.admId}).then(result=>{
...
//able to retrieve ednotes db from master to student interface already
router.get('/ednotes/:recordID/:patientID', ensureAuthenticated, (req, res) => {
	...
	PatientStudentModel.findOne({recordID: req.params.recordID})
	.then(retrieveSPatient => {
		PatientMasterModel.findOne({patientID: retrieveSPatient.patientID})
		.then(retrievedMPatient => {
			// if(JSON.stringify(retrievedPatient.user._id) === JSON.stringify(req.user.id)){
				EDNotesModel.findById(retrievedMPatient.EDNotes_ID, {
				}).then(ednotes => {
					req.session.ednotes = ednotes;
					res.render('patient/master/ednotes', {
						ednotes: ednotes,
						// ednotes: req.session.ednotes,
						recordID: req.params.recordID,
						// patient: retrievedMPatient),
						// patient: req.session.patient,
						user: req.user,
						userType: userType,
						currentUserType: req.user.userType,
					...
					console.log("EDNotes_ID for this patient based on PatientMasterModel: ", retrievedMPatient.EDNotes_ID);
				});
		});
	});
});

router.get('/opprocedure/post-op-procedure-notes/:recordID/:patientID', ensureAuthenticated, (req, res) => {
	userType = req.user.userType == 'student';
	if(req.user.userType == 'staff'){
		userType = 'student';
	}
	...
	.then(retrieveSPatient => {
		PatientMasterModel.findOne({patientID: retrieveSPatient.patientID})
		.then(retrievedMPatient => {
			postopProcedureNotesModel.findById(retrievedMPatient.PostOp_ProcedureNotesID, {
			}).then(postopProNotes => {
				...
				console.log('PostOp_ProcedureNotesID for this patient based on PatientMasterModel: ', retrievedMPatient.PostOp_ProcedureNotesID);
			});
		});
	});
});
